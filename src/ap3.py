...
# Constitution-specific features
def process_constitution():
    """
    Load and process the Constitution, splitting it into chunks and storing in ChromaDB.
    """
    constitution_path = "constitution.pdf"  # Specify the path to the file
    text_splitter = RecursiveCharacterTextSplitter(chunk_size=500, chunk_overlap=50)

    try:
        reader = PdfReader(constitution_path)
        content = "\n".join([page.extract_text() for page in reader.pages])
        chunks = text_splitter.split_text(content)

        for i, chunk in enumerate(chunks):
            add_document_to_chromadb(f"constitution-chunk-{i+1}", chunk)
        st.success("Constitution loaded into ChromaDB.")
    except Exception as e:
        st.error(f"Failed to load Constitution: {e}")

def answer_question_about_constitution(question):
    """
    Provide answers to questions about the Constitution based on ChromaDB.
    """
    results = search_in_chromadb(question)
    if results:
...
st.markdown("**Before loading the Constitution, you should ask your question below and only then press the 'Load Constitution' button.**")

user_question = st.text_input("Ask a question about the Constitution:")
if user_question:
    answer_question_about_constitution(user_question)
